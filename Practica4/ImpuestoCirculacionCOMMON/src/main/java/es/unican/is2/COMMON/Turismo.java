...
public class Turismo extends Vehiculo {
	...
	public double precioImpuesto() {
		...
		if ((getFechaMatriculacion().isBefore(LocalDate.now().minusYears(4)) || getFechaMatriculacion().isEqual(LocalDate.now().minusYears(4)))
				&& getMotor() == TipoMotor.HIBRIDO) { 
			impuesto = impuesto * 0.75;
		}
		if ((getFechaMatriculacion().isBefore(LocalDate.now().minusYears(1)) || getFechaMatriculacion().isEqual(LocalDate.now().minusYears(1))) 
				&& getMotor() == TipoMotor.GAS) {
			impuesto = impuesto * getMotor().descuentoImpuesto;
		}
		return impuesto;
	}
